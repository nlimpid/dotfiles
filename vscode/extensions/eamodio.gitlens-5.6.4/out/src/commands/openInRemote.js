...
class OpenInRemoteCommand extends common_1.ActiveEditorCommand {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                switch (args.resource.type) {
                    case gitService_1.RemoteResourceType.Branch:
                        this.ensureRemoteBranchName(args);
                        placeHolder = `open ${args.resource.branch} branch in${constants_1.GlyphChars.Ellipsis}`;
                        break;
                    case gitService_1.RemoteResourceType.Commit:
                        const shortSha = gitService_1.GitService.shortenSha(args.resource.sha);
                        placeHolder = `open commit ${shortSha} in${constants_1.GlyphChars.Ellipsis}`;
                        break;
                    case gitService_1.RemoteResourceType.File:
                        placeHolder = `open ${args.resource.fileName} in${constants_1.GlyphChars.Ellipsis}`;
                        break;
                    case gitService_1.RemoteResourceType.Revision:
                        if (args.resource.commit !== undefined && args.resource.commit instanceof gitService_1.GitLogCommit) {
                            ...
                            const shortFileSha = args.resource.sha === undefined ? '' : gitService_1.GitService.shortenSha(args.resource.sha);
    ...
    ensureRemoteBranchName(args) {
        if (args.remotes === undefined || args.resource === undefined || args.resource.type !== 'branch')
            return;
