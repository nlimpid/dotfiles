...
class OpenCommitFileRevisionCommandQuickPickItem extends common_1.OpenFileCommandQuickPickItem {
    constructor(commit, item) {
        let description;
        let uri;
        if (commit.status === 'D') {
            uri = gitService_1.GitService.toGitContentUri(commit.previousSha, commit.previousFileName, commit.repoPath, undefined);
...
exports.OpenCommitFileRevisionCommandQuickPickItem = OpenCommitFileRevisionCommandQuickPickItem;
class CommitFileDetailsQuickPick {
    static show(git, commit, uri, goBackCommand, currentCommand, fileLog) {
        return __awaiter(this, void 0, void 0, function* () {
            const items = [];
            const stash = commit.type === gitService_1.GitCommitType.Stash;
            ...
            const pick = yield vscode_1.window.showQuickPick(items, {
                ...
                placeHolder: `${commit.getFormattedPath()} ${system_1.Strings.pad(constants_1.GlyphChars.Dot, 1, 1)} ${isUncommitted ? `Uncommitted ${constants_1.GlyphChars.ArrowRightHollow} ` : ''}${commit.shortSha} ${system_1.Strings.pad(constants_1.GlyphChars.Dot, 1, 1)} ${commit.author}, ${commit.fromNow()} ${system_1.Strings.pad(constants_1.GlyphChars.Dot, 1, 1)} ${commit.message}`,
