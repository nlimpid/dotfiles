...
class CommitQuickPickItem {
    constructor(commit) {
        ...
        if (commit instanceof gitService_1.GitStashCommit) {
            this.label = message;
            this.description = '';
            this.detail = `${constants_1.GlyphChars.Space} ${commit.stashName} ${system_1.Strings.pad(constants_1.GlyphChars.Dot, 1, 1)} ${commit.fromNow()} ${system_1.Strings.pad(constants_1.GlyphChars.Dot, 1, 1)} ${commit.getDiffStatus()}`;
        }
        else {
            this.label = message;
            this.description = `${system_1.Strings.pad('$(git-commit)', 1, 1)} ${commit.shortSha}`;
            this.detail = `${constants_1.GlyphChars.Space} ${commit.author}, ${commit.fromNow()}${(commit.type === gitService_1.GitCommitType.Branch) ? ` ${system_1.Strings.pad(constants_1.GlyphChars.Dot, 1, 1)} ${commit.getDiffStatus()}` : ''}`;
        }
    }
}
