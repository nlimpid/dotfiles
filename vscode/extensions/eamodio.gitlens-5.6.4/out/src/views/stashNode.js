...
class StashNode extends explorerNode_1.ExplorerNode {
    constructor(commit, context, git) {
        ...
        this.git = git;
        this.resourceType = 'gitlens:stash';
    }
    getChildren() {
        return __awaiter(this, void 0, void 0, function* () {
            const statuses = this.commit.fileStatuses;
            const log = yield this.git.getLogForRepo(this.commit.repoPath, `${this.commit.stashName}^3`, 1);
            if (log !== undefined) {
            ...
            const children = statuses.map(s => new stashFileNode_1.StashFileNode(s, this.commit.toFileCommit(s), this.context, this.git));
            children.sort((a, b) => a.label.localeCompare(b.label));
            return children;
        });
    }
    getTreeItem() {
        const item = new vscode_1.TreeItem(gitService_1.CommitFormatter.fromTemplate(this.git.config.gitExplorer.stashFormat, this.commit, {
