...
class Command extends vscode_1.Disposable {
    ...
    static _parseContext(command, options, ...args) {
...
class EditorCommand extends vscode_1.Disposable {
    constructor(command) {
        ...
        const subscriptions = [];
        for (const cmd of command) {
            subscriptions.push(vscode_1.commands.registerTextEditorCommand(cmd, (editor, edit, ...args) => this._execute(cmd, editor, edit, ...args), this));
        }
        this._disposable = vscode_1.Disposable.from(...subscriptions);
    }
    dispose() {
    ...
    _execute(command, editor, edit, ...args) {
        telemetry_1.Telemetry.trackEvent(command);
