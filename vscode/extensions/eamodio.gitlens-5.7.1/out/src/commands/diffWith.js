            ...
            if (commit2 === undefined) {
                if (commit1.isUncommitted) {
                    ...
                    args = {
                        repoPath: commit1.repoPath,
                        lhs: {
                            sha: commit1.previousSha !== undefined ? commit1.previousSha : gitService_1.GitService.fakeSha,
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const [lhs, rhs] = yield Promise.all([
                    args.lhs.sha !== '' && !gitService_1.GitService.isUncommitted(args.lhs.sha)
                        ? this.git.getVersionedFile(args.repoPath, args.lhs.uri.fsPath, args.lhs.sha)
                        : args.lhs.uri.fsPath,
                    args.rhs.sha !== '' && !gitService_1.GitService.isUncommitted(args.rhs.sha)
                ...
                else if (lhs === undefined || args.lhs.sha === gitService_1.GitService.fakeSha) {
                    rhsPrefix = 'added in ';
                }
                if (args.lhs.title === undefined && lhs !== undefined && args.lhs.sha !== gitService_1.GitService.fakeSha) {
                    args.lhs.title = (args.lhs.sha === '' || gitService_1.GitService.isUncommitted(args.lhs.sha))
                        ? `${path.basename(args.lhs.uri.fsPath)}`
                        : `${path.basename(args.lhs.uri.fsPath)} (${gitService_1.GitService.shortenSha(args.lhs.sha)})`;
                }
                if (args.rhs.title === undefined && args.rhs.sha !== gitService_1.GitService.fakeSha) {
                    args.rhs.title = (args.rhs.sha === '' || gitService_1.GitService.isUncommitted(args.rhs.sha))
                        ? `${path.basename(args.rhs.uri.fsPath)}`
                        : `${path.basename(args.rhs.uri.fsPath)} (${rhsPrefix}${gitService_1.GitService.shortenSha(args.rhs.sha)})`;
                }
                ...
                return yield vscode_1.commands.executeCommand(constants_1.BuiltInCommands.Diff, lhs === undefined
                    ? gitService_1.GitService.toGitContentUri(gitService_1.GitService.fakeSha, args.lhs.uri.fsPath, args.repoPath)
                    : vscode_1.Uri.file(lhs), rhs === undefined
                    ? gitService_1.GitService.toGitContentUri(gitService_1.GitService.fakeSha, args.rhs.uri.fsPath, args.repoPath)
