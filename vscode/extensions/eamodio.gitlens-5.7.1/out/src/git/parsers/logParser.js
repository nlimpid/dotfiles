...
class GitLogParser {
        ...
        while (true) {
            ...
            switch (lineParts[0]) {
                ...
                case 'summary':
                    ...
                    break;
                case 'filename':
                    if (type === git_1.GitCommitType.Branch) {
                        next = lines.next();
                        if (next.done)
                            break;
                        i++;
                    ...
                    if (first && repoPath === undefined && type === git_1.GitCommitType.File && fileName !== undefined) {
    ...
    static _parseEntry(entry, type, repoPath, relativeFileName, commits, authors, recentCommit) {
        ...
        if (commit === undefined) {
            ...
            commit = new git_1.GitLogCommit(type, repoPath, entry.sha, relativeFileName, entry.author, new Date(entry.authorDate * 1000), entry.summary, entry.status, entry.fileStatuses, undefined, entry.originalFileName);
        ...
        if (recentCommit !== undefined) {
            ...
            if (type === git_1.GitCommitType.File) {
