...
var RepoChangedReasons;
(function (RepoChangedReasons) {
    RepoChangedReasons["Remotes"] = "remotes";
    RepoChangedReasons["Stash"] = "stash";
    RepoChangedReasons["Unknown"] = "";
})(RepoChangedReasons = exports.RepoChangedReasons || (exports.RepoChangedReasons = {}));
class GitService extends vscode_1.Disposable {
    constructor(repoPath) {
        ...
        this._focused = true;
        this._unfocusedChanges = { repo: false, fs: false };
        ...
        const subscriptions = [
            vscode_1.window.onDidChangeWindowState(this._onWindowStateChanged, this),
            vscode_1.workspace.onDidChangeConfiguration(this._onConfigurationChanged, this),
            factory_1.RemoteProviderFactory.onDidChange(this._onRemoteProviderChanged, this)
        ];
    ...
    _onConfigurationChanged() {
        ...
        if (!system_1.Objects.areEquivalent(cfg.advanced, this.config && this.config.advanced)) {
            if (cfg.advanced.caching.enabled) {
                ...
                const subscriptions = [
                    vscode_1.workspace.onDidCloseTextDocument(d => this._removeCachedEntry(d, RemoveCacheReason.DocumentClosed)),
                    vscode_1.workspace.onDidChangeTextDocument(this._onTextDocumentChanged, this),
                    vscode_1.workspace.onDidSaveTextDocument(d => this._removeCachedEntry(d, RemoveCacheReason.DocumentSaved)),
                ...
                this._cacheDisposable = vscode_1.Disposable.from(...subscriptions);
            }
            else {
                this._cacheDisposable && this._cacheDisposable.dispose();
                this._cacheDisposable = undefined;
    ...
    _fireRepoChange(reason = RepoChangedReasons.Unknown) {
    ...
    isTracked(fileNameOrUri, repoPath) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            logger_1.Logger.log(`isTracked('${fileName}', '${repoPath}')`);
            let tracked = this._trackedCache.get(cacheKey);
            if (tracked !== undefined)
                return tracked;
            const result = yield git_1.Git.ls_files(repoPath === undefined ? '' : repoPath, fileName);
            tracked = !!result;
    ...
    static toGitContentUri(shaOrcommitOrUri, fileName, repoPath, originalFileName) {
        ...
        return vscode_1.Uri.parse(`${constants_1.DocumentSchemes.GitLensGit}:${parsed.dir}${parsed.name}:${shortSha}${parsed.ext}?${JSON.stringify(data)}`);
    }
    static _toGitUriData(commit, originalFileName) {
