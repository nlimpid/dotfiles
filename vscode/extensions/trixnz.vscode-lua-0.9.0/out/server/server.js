...
class ServiceDispatcher {
    ...
    onWorkspaceSymbol(handler) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const uri = lib_1.default.parse(this.rootUri);
    ...
    onCompletion(textDocumentPosition) {
        ...
        const suggestionService = new completionService_1.CompletionService(analysis, textDocumentPosition.position);
    ...
    onDocumentFormatting(params) {
        ...
        const document = this.documents.get(uri);
        return formatService_1.buildDocumentFormatEdits(uri, document, this.settings.format);
    }
    onDocumentRangeFormatting(params) {
        const uri = params.textDocument.uri;
        const document = this.documents.get(uri);
        return formatService_1.buildDocumentRangeFormatEdits(uri, document, params.range, this.settings.format);
    }
    parseAndLintDocument(document) {
        return __awaiter(this, void 0, void 0, function* () {
            const documentUri = document.uri;
            const documentText = document.getText();
            const parsedUri = lib_1.default.parse(documentUri);
            ...
            try {
                const lintingErrors = yield lintingService_1.buildLintingErrors(this.settings, documentUri, documentText);
